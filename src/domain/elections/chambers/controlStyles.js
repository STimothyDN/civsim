...
function mutedPartyColor(color) {
  return `color-mix(in srgb, ${color} 64%, var(--text-muted))`
}
...
export function controlStyleVars(control, prefix = 'winner', partyMeta = PARTY_META) {
  ...
  const baseColor = isEmpty ? 'var(--accent)' : isMinority ? mutedPartyColor(color) : color
