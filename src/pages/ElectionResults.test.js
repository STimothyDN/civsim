...
describe('election result pages', () => {
  ...
  it('renders pre-election climate controls and polls', async () => {
    ...
    expect(wrapper.text()).toContain('Imperial Gazette Polling')
    expect(wrapper.text()).toContain('Rural Voice Network')
    expect(wrapper.text()).toContain('Independent Consensus')
    expect(wrapper.text()).toContain('Asm Total')
    expect(wrapper.text()).toContain('Council Total')
    expect(wrapper.findAll('.pollster-card')).toHaveLength(10)
    expect(wrapper.findAll('.pollster-party-row')).toHaveLength(70)
